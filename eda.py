            ...
            [
                ...
                pd.DataFrame(
                    [
                        {
                            "variable": column,
                            "nombre_val_aberrantes": len(valeurs_aberrantes),
                        }
                    ]
                ),
            ]
...
def clean_na(df):
    ...
    for index, row in df_unique.iterrows():
        ...
        if row["Percentage_of_missing"] > 0.6:
            ...
            colname = str(row["Column_name"])
            df[str(colname) + "_encoded"] = label_encoder.fit_transform(
...
def pieplot_chauffage(bd_dpe):
    ...
    axes[0].pie(
        x=nouveaux_pourcentages_chauffage,
        labels=nouveaux_pourcentages_chauffage.index,
        startangle=45,
        colors=sns.color_palette("pastel", len(nouveaux_pourcentages_chauffage)),
        autopct="%1.1f%%",
    )
    ...
    axes[1].pie(
        x=nouveaux_pourcentages_ECS,
        labels=nouveaux_pourcentages_ECS.index,
        startangle=45,
        colors=sns.color_palette("pastel", len(nouveaux_pourcentages_ECS)),
        autopct="%1.1f%%",
    )
...
def barplot_chauffage(bd_dpe):
    ...
    ax1.bar(
        comptage_type_chauffage.index,
        comptage_type_chauffage,
        color=colors_chauffage,
        label="Chauffage",
    )
    ax1.set_title("Répartition des types d'énergies pour chauffage")
    ax1.set_xticklabels(
        comptage_type_chauffage.index, rotation=45, ha="right"
    )  # Incliner les noms

    # Barres pour 'type_principale_energie_ECS'
    ax2.bar(
        comptage_type_ECS.index,
        comptage_type_ECS,
        color=colors_ECS,
        label="ECS",
        alpha=0.7,
    ...
    ax2.set_xticklabels(
        comptage_type_ECS.index, rotation=45, ha="right"
    )  # Incliner les noms
