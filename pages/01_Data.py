...
st.write(
    "**Pour avoir une description d'une variable vous pouvez la sélectionner dans la liste déroulante :** "
)
...
st.write(
    f"Vous avez sélectionné *{selected_option}*, \n",
...
st.write(
    "Nous avons réalisé un outil qui permet d'extraire les données de la base de données de manière rapide et modulaire depuis l'API de l'Ademe en Open Access. cela nous permet de choisir le nombre d'observations et les variables"
)
...
st.caption(
    f"Données relatives au dépenses énergétiques des logements en France. Nombre d'observation : **{nb_obs}** et nombre de variables : **{nb_col}**."
)
st.write(
    "L'enjeux à présent et de manipuler les données, observer et rectifier les erreurs, supprimer les variables incomplete ou inutilisables."
)
st.write(
    "Pour ce faire, nous utilisons dans un premier temps une fonction que nous avons developpé afin d'effectuer un audit rapides des variables de notres base"
)
st.write(
    "Voici un audit rapide des données afin de mettre en évidence les problème de données de notre base : "
)
...
def pieplot_chauffage_interact(bd_dpe):
    ...
    nouveaux_pourcentages_chauffage = pourcentages_chauffage[
        pourcentages_chauffage >= seuil
    ]
    ...
    # Graphiques
    fig_chauffage = go.Figure(
        go.Pie(
            labels=nouveaux_pourcentages_chauffage.index,
    ...
    fig_ECS = go.Figure(
        go.Pie(
            labels=nouveaux_pourcentages_ECS.index,
    ...
    # Mise en page
    fig_chauffage.update_layout(
        title_text="Répartition types d'énergies pour chauffage des logements",
        width=1000,
    )
    fig_ECS.update_layout(
        title_text="Répartition types d'énergies pour ECS des logements", width=1000
    )
...
def barplot_chauffage_inter(bd_dpe):
    ...
    fig_chauffage.update_layout(
        xaxis=dict(
            tickangle=-20,
            tickmode="array",
            tickvals=list(comptage_type_chauffage.index),
        )
    )
    ...
    fig_ECS.update_layout(
        xaxis=dict(
            tickangle=-45, tickmode="array", tickvals=list(comptage_type_ECS.index)
        )
    )
...
tab1_chauffage, tab2_chauffage = st.tabs(
    ["📈 Pieplot", "📊 Barplot"]
)  # POUR ENERGIE CHUAFFAGE
