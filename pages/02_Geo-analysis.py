...
st.write(
    "Dans cette section, nous explorerons les données d’un point de vue géographique. Nous générerons plusieurs cartes à différentes échelles pour mettre en évidence diverses variables d'intérêt."
)
...
def create_gdf(DPE_data):
    gdf = gpd.GeoDataFrame(
        ...
        geometry=gpd.points_from_xy(
            DPE_data["Coordonnée_cartographique_X_(BAN)"],
            DPE_data["Coordonnée_cartographique_Y_(BAN)"],
        ),
        crs="EPSG:2154",
    )
    ...
    gdf = gdf.drop(
        gdf[
            (gdf["Coordonnée_cartographique_X_(BAN)"] == x_point_to_remove)
            & (gdf["Coordonnée_cartographique_Y_(BAN)"] == y_point_to_remove)
        ].index
    )
    return gdf
...
def carte_france(gdf, selected_var="Conso_5_usages/m²_é_finale"):
    ...
    gdf.to_crs(3857).plot(
        column=selected_var, cmap="viridis", ax=ax, alpha=0.4, zorder=2, legend=True
    )
...
choix1 = col1.radio(
    ...
    captions=[
        "consommation d'énergie primaire totale rapportée à la surface (kWhep/m²/an)",
...
st.write(
    "###Dans un second temps, apres avoir representé la France de manière globale nous nous interessons au découpage départemental et à la répartition des logements de notre base dans des zones spécifiquement sélectionnées"
)
...
st.caption(
    f"Carte de la sélection de départements : {codes_dep}; Variable sélectionnée : ({interest_var})"
)
...
def carte_region(gdf, region_choisie, var="Conso_5_usages/m²_é_finale"):
    ...
    shp_communes = s3.download_vectorfile_url_all(
        ...
        values=gdf_region[
            "code"
        ].unique(),  # on obtiens tous les codes des département de la régions selectionnée
    ...
    # gdf_paris
    fig, ax = plt.subplots(figsize=(10, 10))
    gdf_region.to_crs(3857).plot(
        ax=ax,
        column="Conso_5_usages/m²_é_finale",
        legend=True,
        edgecolor="black",
        linewidth=0.5,
        zorder=2,
    ...
    gdf_region["geometry_y"].to_crs(3857).plot(
        ax=ax, marker="o", markersize=1, alpha=0.7, color="red", zorder=4
    )

    shp_communes.to_crs(3857).boundary.plot(
        ax=ax,
        zorder=3,
        edgecolor="black",
        linewidth=0.5,
...
DPE_data = DPE_data.merge(
    Noms_regions, left_on="N°_région_(BAN)", right_on="Nouveau Code"
).drop("Nouveau Code", axis=1)
...
def graphiques_conso_reg(DPE_data):
    ...
    fig1 = px.bar(
        val_zone_climatique,
        x=val_zone_climatique.index,
        y=val_zone_climatique.values,
        color=val_zone_climatique.index,
    ...
    fig2 = px.bar(
        ...
        x=val_region.index,
        y=val_region.values,
        labels={"x": "Région", "y": "Count"},
        color=val_region.index,
    )
    ...
    comptage_conso_region = DPE_data.groupby("Nouveau Nom")[
        "Conso_5_usages_é_finale"
    ].sum()
    fig3 = px.bar(
        comptage_conso_region,
        x=comptage_conso_region.index,
        y=comptage_conso_region.values,
        labels={"x": "Région", "y": "Sum of consomation"},
        color=comptage_conso_region.index,
    )
    fig3.update_layout(title="Sum of consomation by Region", xaxis_tickangle=-45)

    tab1_nblogements, tab2_conso = st.tabs(
        [
            "Répartition des logements par région",
            "Classement des régions par consommation",
        ]
    ...
    df_conso_regions["conso_moyenne"] = (
        df_conso_regions["Conso_5_usages_é_finale"] / df_conso_regions["count"]
    )

    fig = go.Figure()  # go.Scatterpolar pour créer un graphique radar
    fig.add_trace(
        go.Scatterpolar(
            r=df_conso_regions["conso_moyenne"],
    ...
    fig.update_layout(
        ...
        title="Radar Chart of Mean Consumption",
        height=600,
    )
