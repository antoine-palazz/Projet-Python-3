...
DPE_data["Type_énergie_principale_ECS"].fillna(
    DPE_data["Type_énergie_n°1"], inplace=True
)
DPE_data["Type_énergie_principale_chauffage"].fillna(
    DPE_data["Type_énergie_n°1"], inplace=True
)
DPE_data["Type_énergie_principale_chauffage"] = DPE_data[
    "Type_énergie_principale_chauffage"
].replace(
    [
        "Bois – Bûches",
...
DPE_data["Type_énergie_principale_ECS"] = DPE_data[
    "Type_énergie_principale_ECS"
].replace(
    [
        "Bois – Bûches",
...
DPE_data = (
    DPE_data.dropna()
)  # on termine par suprimmer les lignes avec des valeusr manquantes
...
def barplot_chauffages(bd_dpe):
    ...
    plt.bar(
        index, comptage_type_ECS, bar_width, color=colors_ECS, label="ECS", alpha=0.7
    )
